"""Single Alpha: EWMAC_Ensemble -- Equal-weight blend of 3 EWMAC speeds with TREND_FDM=1.10"""
import sys
import numpy as np
import pandas as pd
...
STRATEGY_NAME = "Single_Alpha_EWMAC_Ensemble"
OOS_START = 1280
...
def run_strategy():
    ...
    for instrument in tqdm(mapping.index, desc=STRATEGY_NAME):
        ...
        # -- SIGNAL: 3-speed EWMAC ensemble, equal-weight, TREND_FDM=1.10 --
        components = []
        ...
        blended = (components[0] + components[1] + components[2]) * TREND_FDM
        forecast_scaled = cap_forecast(blended.fillna(0.0))
