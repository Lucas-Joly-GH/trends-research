"""
Single Alpha: Tail Asymmetry -- Upper vs Lower tail risk ratio
================================================================
Signal: (UpperTail - LowerTail) from rolling statistics.
UpperTail > LowerTail = positive tail asymmetry = go long.
LowerTail > UpperTail = negative tail asymmetry = go short.

Orthogonal to Skew (3rd moment) because it captures extremes of the
distribution (tail probabilities), not the bulk asymmetry.

Reference: Kelly & Jiang (2014) "Tail Risk and Asset Prices";
Bollerslev et al. (2015) "Tail Risk Premia and Return Predictability".
"""
import sys
import os
import numpy as np
import pandas as pd
from pathlib import Path
from tqdm import tqdm
import warnings
warnings.filterwarnings("ignore")

_HERE = Path(__file__).resolve().parent
sys.path.insert(0, str(_HERE.parent))
from ig_shared_config import (
    FORECAST_CAP, FORECAST_TARGET, VOL_TARGET,
    get_project_paths, load_mapping, load_fx_rates, load_instrument_data,
    align_fx, blended_volatility, cap_forecast, run_compounded_portfolio,
    rolling_forecast_scalar,
)

STRATEGY_NAME = "Single_Alpha_Tail_Asymmetry"
OOS_START = 1280


def _load_tails(instrument, stats_folder, dates):
    """Load UpperTail and LowerTail from statistics file."""
    stats_file = os.path.join(stats_folder, f"{instrument}_statistics.csv")
    if not os.path.exists(stats_file):
        return None, None
    df = pd.read_csv(stats_file, usecols=["Date", "UpperTail", "LowerTail"],
                     parse_dates=["Date"])
    df = df.dropna(subset=["UpperTail", "LowerTail"]).sort_values("Date").set_index("Date")
    ut = df["UpperTail"].reindex(dates).ffill()
    lt = df["LowerTail"].reindex(dates).ffill()
    return ut, lt


def run_strategy():
    paths    = get_project_paths(_HERE)
    mapping  = load_mapping(paths["mapping"])
    fx_daily = load_fx_rates(paths["panama"])

    inst_signals = {}
    for instrument in tqdm(mapping.index, desc=STRATEGY_NAME):
        data = load_instrument_data(instrument, paths["stats"], paths["panama"])
        if data is None:
            continue
        dates = data["daily_dates"]
        if len(dates) < OOS_START + 2:
            continue
        ccy = mapping.loc[instrument, "currency"]
        fx  = align_fx(ccy, dates, fx_daily)
        if fx is None:
            continue

        price_changes = data["price_changes"]
        vol           = blended_volatility(price_changes)
        idx           = pd.DatetimeIndex(dates)

        ut, lt = _load_tails(instrument, paths["stats"], idx)
        if ut is None:
            inst_signals[instrument] = {
                "oos_date_set": set(dates[OOS_START:]),
                "forecast":     pd.Series(0.0, index=idx),
                "vol":          vol.reindex(idx),
                "fx":           fx.reindex(idx),
                "close":        data["close"].reindex(idx),
                "open":         data["open"].reindex(idx),
                "pointsize":    float(mapping.loc[instrument, "pointsize"]),
                "cost_rt":      float(mapping.loc[instrument, "total_avg_cost_rt"]),
            }
            continue

        # -- SIGNAL: Tail asymmetry = UpperTail - LowerTail --
        raw = (ut - lt).fillna(0.0)
        forecast_scaled = cap_forecast(raw * rolling_forecast_scalar(raw))

        final_fc = forecast_scaled.reindex(idx).fillna(0.0)
        final_fc.iloc[:OOS_START] = 0.0

        inst_signals[instrument] = {
            "oos_date_set": set(dates[OOS_START:]),
            "forecast":     final_fc,
            "vol":          vol.reindex(idx),
            "fx":           fx.reindex(idx),
            "close":        data["close"].reindex(idx),
            "open":         data["open"].reindex(idx),
            "pointsize":    float(mapping.loc[instrument, "pointsize"]),
            "cost_rt":      float(mapping.loc[instrument, "total_avg_cost_rt"]),
        }

    run_compounded_portfolio(inst_signals, STRATEGY_NAME, paths)


if __name__ == "__main__":
    run_strategy()
