...
# WHERE THE SINGLE-ALPHA INPUTS LIVE.  They sit in the thesis repo, which is
# private and is not a sibling of this one on every machine -- so the location
# is configurable and the default merely guesses that the two repos are checked
# out side by side.  This used to be an absolute path with a personal account
# name baked into it, which is nobody's business in a public repo and broke for
# every reader anyway.
DATA = pathlib.Path(os.environ.get(
    "SINGLE_ALPHA_DIR",
    pathlib.Path(__file__).resolve().parents[2] / "LJOLY_Memoire_INSEEC_Msc2"
    / "Part 3" / "IG_Backtest" / "Single_Alpha"))

corr_df = pd.read_csv(DATA / "single_alpha_correlation_matrix.csv", index_col=0)
summary = pd.read_csv(DATA / "single_alpha_summary.csv")
