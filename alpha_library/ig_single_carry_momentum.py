"""Single Alpha: Carry_Momentum -- 63-day change in raw carry level (Koijen et al. 2018)"""
import sys
import numpy as np
import pandas as pd
...
STRATEGY_NAME = "Single_Alpha_Carry_Momentum"
OOS_START = 1280
...
def run_strategy():
    ...
    for instrument in tqdm(mapping.index, desc=STRATEGY_NAME):
        ...
        # -- SIGNAL: 63-day change in raw carry --
        ...
        if raw_carry is not None and raw_carry.dropna().any():
            raw = raw_carry.fillna(0.0).diff(63).fillna(0.0)
