"""
Single Alpha: Kurtosis -- Inverted rolling excess kurtosis (256-day)
=====================================================================
High kurtosis = fat tails = upcoming regime change -> fade the current trend.
Low kurtosis = thin tails = stable regime -> ride the trend.
Signal: -kurtosis (inverted: go short when kurtosis high, long when low).

Rationale: Orthogonal to trend & carry because it captures distributional
shape, not direction or yield. Similar spirit to Skew but captures a
different moment (4th vs 3rd).

Reference: Harvey & Siddique (2000), Amaya et al. (2015) "Does Realized
...
STRATEGY_NAME = "Single_Alpha_Kurtosis"
OOS_START = 1280
KURT_WINDOW = 256
...
def run_strategy():
    ...
    for instrument in tqdm(mapping.index, desc=STRATEGY_NAME):
        ...
        # -- SIGNAL: Inverted rolling excess kurtosis --
        # High kurtosis = fat tails = fade; Low kurtosis = ride
        raw = -price_changes.rolling(KURT_WINDOW, min_periods=128).kurt().fillna(0.0)
