"""
Single Alpha: Volume Momentum -- Rate of change of volume
==========================================================
Rising volume = confirmation of current price trend.
Signal: EMA(21) of normalised daily volume change (vol today / vol_avg - 1).
Then multiply by sign(EWMAC) to get directional signal.

Without directional overlay, pure volume momentum has no sign.
We use: sign(63-day return) * normalised_volume_change.

Rationale: Volume carries information about conviction and participation
that is orthogonal to price-based trend signals.

Reference: Llorente et al. (2002) "Dynamic Volume-Return Relation of
Individual Stocks"; Karpoff (1987) "The Relation Between Price Changes
and Trading Volume".
"""
import sys
import os
...
STRATEGY_NAME = "Single_Alpha_Volume_Momentum"
OOS_START = 1280
...
def run_strategy():
    ...
    for instrument in tqdm(mapping.index, desc=STRATEGY_NAME):
        ...
        # -- SIGNAL: Volume momentum * price direction --
        vol_avg = volume.rolling(63, min_periods=21).mean()
        vol_norm = ((volume / vol_avg) - 1.0).replace([np.inf, -np.inf], np.nan).fillna(0.0)
        vol_smooth = vol_norm.ewm(span=21).mean()

        # Direction from 63-day return
        direction = np.sign(close.pct_change(63).fillna(0.0))

        raw = (direction * vol_smooth).fillna(0.0)
